...
require(len(route_exports) == 39, f"expected 39 canonical Worker route constants, found {len(route_exports)}")
registry_rows = re.findall(r'\{\s*match:\s*"[^"]+",\s*path:\s*"[^"]+",\s*domain:\s*"[^"]+",\s*handler:\s*"[^"]+",\s*access:\s*"[^"]+"', registry)
require(len(registry_rows) == 40, f"expected 40 Worker route registry rows, found {len(registry_rows)}")
