...
test('existing homepage image slots use clean, localized, or explicit verification-hold media without changing layout rules', async () => {
  const body = await (await request('/')).text();
  ...
  assert.match(css, /home-product-card__image:has\(img\[src\^="http"\]\)/);
  assert.match(css, /VERIFICATION PENDING/);
...
test('current catalog presentation remains styled without reintroducing rejected homepage visual overrides', async () => {
  const store = await (await request('/store')).text();
  const css = await (await request('/assets/app.css')).text();
  assert.match(store, /class="store-hero"/);
  assert.match(store, /class="store-category-grid"/);
  assert.match(store, /class="catalog-grid catalog-featured-grid"/);
  assert.match(css, /\.store-hero\{position:relative;min-height:570px/);
  assert.match(css, /\.store-category-grid\{display:grid;grid-template-columns:repeat\(6,1fr\)/);
  assert.match(css, /\.catalog-card-media\{display:block;height:190px/);
  assert.match(css, /\.store-category-icon-visual \.semantic-icon\{width:78px;height:78px/);
  assert.doesNotMatch(css, /reference-storefront-home \.solution-grid\{grid-template-columns:repeat\(7/);
});
