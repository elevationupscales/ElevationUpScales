...
for i,line in enumerate(lines):
    if line.startswith("| Checkout-gate cleanup — authorized preorder/backorder and unnecessary internal gates |"):
        lines[i]=replacement; found=True
    if line.startswith("**Last source-state reconciliation baseline:**"):
        lines[i]="**Last source-state reconciliation baseline:** `ec65338dcfd0526f8d5c5d0269d2f0bb5566d9ce`"
if not found: raise SystemExit("checkout-gate row not found")
path.write_text("\n".join(lines)+"\n")
