 ...
 * Canonical owner shell for Situation -> Priority -> Action.
 * Marketplace retired 2026-09-07; secondary commerce managers are nested under owners.
 */
(() => {
  "use strict";
  ...
  async function settle(name, endpoint) {
    try { return { name, ok: true, data: await api(endpoint) }; }
    catch (error) { return { name, ok: false, status: error.status || 0, error: error.message }; }
  }

  async function loadAll() {
    const results = await Promise.all(Object.entries(ENDPOINTS).map(([name, endpoint]) => settle(name, endpoint)));
  ...
  function orderActions(orders = []) {
    ...
    return {
      ...
      tracking: list.filter((order) => order.fulfillmentStatus === "supplier_ordered" && !text(order.trackingNumber)).length,
      ...
      open: list.filter((order) => !["completed", "refunded", "cancelled"].includes(order.fulfillmentStatus)).length,
    };
  }
  ...
  function leadActions(projects = []) {
    ...
    return {
      ...
      followup: active.filter((project) => text(project.pipelineStatus || project.status).toLowerCase() === "follow_up").length,
  ...
  function computeRevenue(orders = []) {
    const paid = (Array.isArray(orders) ? orders : []).filter((order) => !["cancelled", "refunded"].includes(order.fulfillmentStatus) && text(order.paymentStatus).toLowerCase() !== "refunded");
    const revenue = paid.reduce((sum, order) => sum + (Number(order.totalCents) || 0), 0);
    const supplierCost = paid.reduce((sum, order) => sum + (Number(order.supplierCostCents) || 0) * (Number(order.quantity) || 1), 0);
  ...
  function valueOrNA(value, formatter = (item) => String(item)) {
    return value === null || value === undefined || Number.isNaN(value) ? "N/A" : formatter(value);
  }
  function numberValue(value) { return Number.isFinite(Number(value)) ? Number(value) : 0; }
  function upperState(value, fallback = "UNKNOWN") { return String(value || fallback).replaceAll("_", " ").toUpperCase(); }
  ...
  async function enhanceOrders() {
    ...
    try {
      ...
      board.innerHTML = items.map(([label, count, status, className]) => `<button class="eus-order-action ${className}" type="button" data-order-quick="${status}" data-tracking-only="${label === "Tracking needed" ? "1" : "0"}"><strong>${count}</strong><span>${label}</span></button>`).join("");
      ...
      board.addEventListener("click", (event) => {
        ...
        document.querySelector("#orders-workspace")?.scrollIntoView?.({ behavior: "smooth" });
      });
    } catch (_) {}
  }

  function buildOverviewModel(snapshot = {}) {
    const dataIfOk = (key, fallback = {}) => snapshot?.[key]?.ok ? snapshot[key].data ?? fallback : fallback;

    const orders = snapshot.orders?.ok ? dataIfOk("orders").orders || [] : [];
    ...
    const projects = snapshot.opportunities?.ok ? dataIfOk("opportunities").projects || [] : [];
    ...
    const batchMetrics = Array.isArray(lithium.metrics?.batchMetrics) ? lithium.metrics.batchMetrics : [];

    const shipping = {
      reservations: snapshot.lithium?.ok ? requests.filter((request) => !["CLOSED", "CANCELLED"].includes(upperState(request.state))).length : null,
      supplierRechecks: snapshot.lithium?.ok ? records.filter((record) => record.inventoryRecheckRequired || ["UNKNOWN", "RECHECK REQUIRED", "SUPPLIER ERROR"].includes(upperState(record.supplierStockState))).length : null,
      blocked: null,
      confirmations: null,
      openBatches: null,
    };
    if (snapshot.lithium?.ok) {
      const blockedLines = batchMetrics.reduce((sum, metric) => sum + numberValue(metric.blockedOrders), 0);
      shipping.blocked = records.filter((record) => record.hold || upperState(record.reviewState) !== "INTERNAL REQUIREMENTS SATISFIED").length + blockedLines;
      shipping.confirmations = batchMetrics.reduce((sum, metric) => sum + numberValue(metric.pendingCustomerConfirmations), 0);
      shipping.openBatches = batches.filter((batch) => !["COMPLETE", "CANCELLED"].includes(upperState(batch.status))).length;
    }

    const syncIssues = snapshot.sync?.ok ? numberValue(syncCounts.outOfSync) + numberValue(syncCounts.syncError) + numberValue(syncCounts.stale) : null;
    const inPortal = snapshot.opportunities?.ok ? projects.filter((project) => project.portalStatus === "in_portal").length : null;
    const handoffReady = snapshot.opportunities?.ok ? projects.filter((project) => Boolean(project.portalHandoffReady)).length : null;
    const wonNotPortal = snapshot.opportunities?.ok ? projects.filter((project) => text(project.pipelineStatus || project.status).toLowerCase() === "won" && project.portalStatus !== "in_portal").length : null;
    const website = snapshot.operations?.ok ? (health.publicHealth === "ok" ? "ONLINE" : upperState(health.publicHealth || health.status)) : "N/A";

    const situation = [
      { label: "Orders", value: snapshot.orders?.ok ? String(orderState.open) : "N/A", note: snapshot.orders?.ok ? `${orderState.supplier} need supplier · ${orderState.tracking} need tracking` : "Orders API unavailable", href: "/admin-store-orders", kind: orderState.hold + orderState.refund ? "is-urgent" : orderState.supplier + orderState.tracking ? "is-warning" : "" },
      { label: "Revenue", value: snapshot.orders?.ok ? revenue.revenue : null, money: true, note: snapshot.orders?.ok ? `${revenue.orders} stored non-refunded orders` : "Stored order value unavailable", href: "/admin-analytics", kind: "" },
      { label: "Leads", value: snapshot.opportunities?.ok ? String(leadState.active) : "N/A", note: snapshot.opportunities?.ok ? `${leadState.new} new · ${leadState.unassigned} unassigned` : "Opportunities API unavailable", href: "/admin-listings#leads", kind: leadState.unassigned + leadState.noNext ? "is-warning" : "" },
      { label: "Products", value: snapshot.catalog?.ok ? `${catalogState.published}/${catalogState.total}` : "N/A", note: snapshot.catalog?.ok ? `${catalogState.review} need review · ${catalogState.drafts} drafts` : "Catalog API unavailable", href: "/admin-catalog", kind: catalogState.review ? "is-warning" : "" },
      { label: "Channels", value: syncIssues === null ? "N/A" : syncIssues ? `${syncIssues} REVIEW` : "CLEAR", note: snapshot.sync?.ok ? `${numberValue(syncCounts.ready)} ready · ${numberValue(syncCounts.outOfSync)} out of sync` : "Sync API unavailable", href: "/admin-channels", kind: syncIssues === 0 ? "is-good" : syncIssues ? "is-warning" : "" },
      { label: "Shipping & Logistics", value: shipping.blocked === null ? "N/A" : shipping.blocked ? `${shipping.blocked} BLOCKED` : "CLEAR", note: shipping.reservations === null ? "Lithium shipping API unavailable" : `${shipping.reservations} reservations · ${shipping.openBatches} open batches`, href: "/admin-lithium-shipping", kind: shipping.blocked === 0 ? "is-good" : shipping.blocked ? "is-warning" : "" },
      { label: "Website", value: website, note: snapshot.operations?.ok ? `Lead Core ${upperState(health.leadCore)} · Build ${health.build || "unknown"}` : "Health signal unavailable", href: "/admin-system", kind: health.publicHealth === "ok" ? "is-good" : snapshot.operations?.ok ? "is-warning" : "" },
    ];
    ...
    const actionable = priorities.filter((item) => item.count !== null && numberValue(item.count) > 0).sort((a, b) => b.weight - a.weight);
    const programs = [
    ...
    return Object.freeze({
      orders, orderState, revenue, catalogState, inventoryStats, leadState, shipping, situation, priorities,
      actionable: actionable.slice(0, 8),
      actionableCount: actionable.reduce((sum, item) => sum + numberValue(item.count), 0),
      actionableTypes: actionable.length,
      programs,
      metrics: {
        storeRevenueCents: snapshot.orders?.ok ? revenue.revenue : null,
        grossContributionCents: snapshot.orders?.ok ? revenue.grossContribution : null,
        averageOrderCents: snapshot.orders?.ok ? revenue.aov : null,
        inventoryValueCents: snapshot.inventory?.ok ? numberValue(inventoryStats.inventoryValueCents) : null,
        lowStock: snapshot.inventory?.ok ? numberValue(inventoryStats.lowStock) : null,
        submittedLeads: snapshot.operations?.ok ? numberValue(signals.submittedLeads) : null,
      },
      leadHealth: {
        ...
        followUpsDue: snapshot.operations?.ok ? numberValue(signals.followUpsDue) : null,
      },
      unavailable: Object.values(snapshot).filter((entry) => !entry?.ok).map((entry) => entry.name).filter(Boolean),
    });
  }

  window.EUSAdminData = Object.freeze({
    api, loadAll, getOrders, getCatalog, getInventory, getOpportunities, getOperations, getAnalytics, getLithium, getSync, getDoba, getPromotion,
    orderActions, leadActions, catalogActions, computeRevenue, money, valueOrNA, text, numberValue, upperState, buildOverviewModel,
  });
