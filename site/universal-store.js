(() => {
  ...
  function sale(p){const publish=text(p.publishStatus||p.publish_status).toLowerCase(),mode=text(p.availabilityMode||p.availabilityStatus||p.status||"available").toLowerCase(),source=text(p.sourceState||p.source_state||p.reviewState||"").toLowerCase(),shipping=text(p.shippingStatus||p.shipping_status||"").toLowerCase(),supplier=ident(p),raw=p.supplierStock??p.inventoryQty??p.inventory_qty??p.quantityOnHand,has=raw!==null&&raw!==undefined&&String(raw)!=="",stock=has?Number(raw):null,price=Number(p.priceCents||p.price_cents||0),eligible=p.paymentEligible!==false;
    ...
    if(/doba/.test(supplier)){if(!has||!Number.isFinite(stock))return{code:"hold",label:"Confirm Availability",buy:false};if(shipping&&shipping!=="verified")return{code:"hold",label:shipping==="quote_required"?"Shipping Quote Required":"Confirm Availability",buy:false};return{code:"buy",label:"Available",buy:price>0};}
  ...
  function action(p,s){const id=text(p.id||""),sku=text(p.sku||p.supplierSku||""),title=compactTitle(p),detail=`/product?id=${encodeURIComponent(id||sku)}&store=universal`,section=text(p.storeSection||p.store_section).toLowerCase(),checkoutSource=p.sokProduct||section==="lithium-batteries"?"lithium":"rv";if(s.buy)return{href:`/checkout/?source=${checkoutSource}&id=${encodeURIComponent(id||sku)}&name=${encodeURIComponent(title)}`,label:/prepurchase|preorder|backorder/i.test(String(p.availabilityMode||""))?"Purchase Options":"Buy Now",detail};if(p.sokProduct)return{href:`/sok-order.html?sku=${encodeURIComponent(sku)}&intent=purchase_options`,label:"See Purchase Options",detail};return{href:detail,label:s.code==="out"?"View Product":"Confirm Availability",detail};}
  function card(p){const s=sale(p),a=action(p,s),title=compactTitle(p),supplier=brand(p),image=mediaUrl(p),desc=text(p.description).slice(0,180),price=Number(p.priceCents||p.price_cents||0),stock=p.supplierStock??p.inventoryQty??p.inventory_qty,inventory=stock===null||stock===undefined?"Check current availability before purchase.":"Ships from supplier inventory.";return`<article class="universal-product"><a class="universal-product__image" href="${esc(a.detail)}"><img src="${esc(image)}" alt="${esc(title)}" loading="lazy" decoding="async" referrerpolicy="no-referrer"></a><div class="universal-product__body"><div class="universal-product__meta">${esc(supplier)} · ${esc(department(p).replaceAll("-"," "))}</div><h3>${esc(title)}</h3>${desc?`<p class="universal-product__description">${esc(desc)}</p>`:""}<span class="universal-product__state is-${esc(s.code)}">${esc(s.label)}</span><p class="universal-product__description">${esc(inventory)}</p><div class="universal-product__footer"><strong class="universal-product__price">${price>0?money.format(price/100):"Purchase options"}</strong><div class="universal-product__actions"><a class="button button-outline" href="${esc(a.detail)}">Details</a><a class="button button-primary" href="${esc(a.href)}">${esc(a.label)}</a></div></div></div></article>`;}
  ...
  function render(){const out=rows();grid.innerHTML=out.length?out.map(card).join(""):`<div class="universal-empty"><strong>No products match this view.</strong><p>Try another department or clear the search.</p></div>`;status.textContent=`${out.length} product${out.length===1?"":"s"}`;chips.forEach(c=>c.classList.toggle("is-active",c.dataset.universalDepartment===state.department));}
  ...
  status.textContent="Loading products…";load().then(p=>{state.products=p;render();}).catch(()=>{status.textContent="Products are temporarily unavailable.";grid.innerHTML='<div class="universal-empty"><strong>We could not load the current catalog.</strong><p>Please try again shortly or contact Elevation for product availability.</p></div>';});
})();