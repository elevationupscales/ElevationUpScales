...
const ORDER_STATUSES = new Set(["pending", "paid", "fulfillment_pending", "supplier_ordered", "shipped", "completed", "refund_needed", "refunded", "cancelled"]);
...
async function requireAdmin(request, env) {
  ...
  if (!session) return { response: json({ error: "Admin login required" }, 401) };
  return { session };
}
...
function normalizedStatus(row) {
  const stored = clean(row?.fulfillment_status, 40).toLowerCase();
  if (ORDER_STATUSES.has(stored) && stored !== "pending") return stored;
  return clean(row?.payment_status, 40).toLowerCase() === "completed" ? "fulfillment_pending" : (ORDER_STATUSES.has(stored) ? stored : "pending");
}

function orderRecord(row) {
  const customer = parseJson(row.customer_json);
  const shipping = parseJson(row.shipping_json);
  const supplier = parseJson(row.supplier_json);
  return {
    id: clean(row.id, 120),
    source: clean(row.source, 30),
    productId: clean(row.product_id, 180),
    productName: clean(row.product_name, 260),
    variantId: clean(row.variant_id, 180),
    variantName: clean(row.variant_name, 220),
    quantity: Number(row.quantity || 0),
    unitPriceCents: Number(row.unit_price_cents || 0),
    merchandiseCents: Number(row.merchandise_cents || 0),
    ...
    fulfillmentNotes: clean(row.fulfillment_notes, 2000),
    createdAt: clean(row.created_at, 50),
    paidAt: clean(row.paid_at, 50),
    updatedAt: clean(row.updated_at, 50),
    refundedAt: clean(row.refunded_at, 50),
  };
}

function counts(orders) {
  const result = { total: orders.length, paid: 0, fulfillment_pending: 0, supplier_ordered: 0, shipped: 0, completed: 0, refund_needed: 0, refunded: 0 };
  for (const order of orders) {
    if (order.paymentStatus === "completed") result.paid += 1;
    if (Object.hasOwn(result, order.fulfillmentStatus)) result[order.fulfillmentStatus] += 1;
  }
  return result;
}
...
async function listOrders(request, env) {
  ...
  const rows = await env.MARKETPLACE_DB.prepare("SELECT * FROM eus_store_orders ORDER BY COALESCE(paid_at, created_at) DESC LIMIT 500").all();
  const orders = (rows.results || []).map(orderRecord);
  const payload = { ok: true, orders, counts: counts(orders), admin: auth.session.email };
...
async function updateOrder(request, env, id) {
  ...
  if (!/^EUS-STORE-\d{8}-[A-F0-9]{8}$/.test(orderId)) return json({ error: "Invalid store order reference" }, 400);
  const body = await request.json().catch(() => ({}));
  const status = clean(body.fulfillmentStatus, 40).toLowerCase();
  if (!ORDER_STATUSES.has(status)) return json({ error: "Invalid fulfillment status" }, 400);
  const supplierOrderId = clean(body.supplierOrderId, 160);
  const trackingNumber = clean(body.trackingNumber, 180);
  const carrier = clean(body.carrier, 120);
  const fulfillmentNotes = clean(body.fulfillmentNotes, 2000);
  const now = new Date().toISOString();
  const refundedAt = status === "refunded" ? now : null;
  const result = await env.MARKETPLACE_DB.prepare(`
    UPDATE eus_store_orders
       SET fulfillment_status=?, supplier_order_id=?, tracking_number=?, carrier=?, fulfillment_notes=?, updated_at=?, refunded_at=COALESCE(?, refunded_at)
     WHERE id=?
  ...
  const row = await env.MARKETPLACE_DB.prepare("SELECT * FROM eus_store_orders WHERE id=? LIMIT 1").bind(orderId).first();
...
export async function handleStoreOrdersAdminApi(request, env, pathname) {
  ...
  if (path === "/api/admin/store-orders") return listOrders(request, env);
  const match = path.match(/^\/api\/admin\/store-orders\/(EUS-STORE-\d{8}-[A-F0-9]{8})$/i);
  if (match) return updateOrder(request, env, match[1].toUpperCase());
