...
const ADMIN_USABILITY_PATHS = new Set([
  "/admin-catalog", "/admin-inventory", "/admin-channels", "/admin-store-orders", "/admin-lithium-shipping"
]);
...
function adminUsabilityResponse(response, pathname) {
  if (!response || response.status !== 200 || !ADMIN_USABILITY_PATHS.has(pathname)) return response;
  const type = String(response.headers.get("Content-Type") || "");
  if (!type.toLowerCase().includes("text/html")) return response;
  return new HTMLRewriter().on("head", {
    element(head) {
      head.append('<link rel="stylesheet" href="/admin-usability-merchandising.css?v=4.3.10"><script defer src="/admin-usability-merchandising.js?v=4.3.10"></script>', { html: true });
    },
  }).transform(response);
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    return adminUsabilityResponse(response,normalizedPath);
  },
};
