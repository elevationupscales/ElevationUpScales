...
import { resolvePropertyOpportunityAttribution, linkPropertyOpportunityLead } from "./property-opportunities.js";
...
async function handleProjectCapture(request,env,ctx){
  ...
  const city=cleanString(b.city,120),zip=normalizeProjectZip(b.zip),state=normalizeProjectState(b.state),contact=safeContactPayload(b),journeyId=cleanString(b.journeyId,120);
  const propertyAttribution=await resolvePropertyOpportunityAttribution(env,b.propertyOpportunitySlug);
  const source=propertyAttribution?"Property Intelligence":(cleanString(b.source,120)||"start-a-project");
  ...
  const area=classifyProjectServiceArea({city,zip,state}),requested=cleanString(b.reference,80),linkedReference=propertyAttribution&&/^EUS-(HOME|RV|SOLAR)-\d{8}-[A-F0-9]{7,8}$/.test(propertyAttribution.convertedLeadId)?propertyAttribution.convertedLeadId:"",reference=/^EUS-(HOME|RV|SOLAR)-\d{8}-[A-F0-9]{8}$/.test(requested)?requested:(linkedReference||projectReference(projectType.toUpperCase())),now=new Date().toISOString(),status=area.serviceArea==="outside_standard_area"?"outside_area_review":"potential",nextAction=area.serviceArea==="outside_standard_area"?"Outside Area Review":"Review Potential Project",propertyIntelligence=propertyAttribution?{opportunityId:propertyAttribution.opportunityId,subtype:propertyAttribution.subtype,campaign:propertyAttribution.campaign,propertySource:propertyAttribution.propertySource,scoreConfidence:propertyAttribution.scoreConfidence,initialScores:{solar:propertyAttribution.solarScore,lithium:propertyAttribution.lithiumScore,combined:propertyAttribution.combinedScore},conceptViewed:propertyAttribution.conceptViewed,qrScanCount:propertyAttribution.qrScanCount}:{},incomingDetails={...(handymanDetails||rawDetails),journeyId,intakeIntent,...(propertyAttribution?{propertyIntelligence}:{})},details=await mergeProjectIntakeDetails(env,reference,incomingDetails);
  try{
  ...
  if(propertyAttribution)await linkPropertyOpportunityLead(env,propertyAttribution.opportunityId,reference,false);
...
async function handleProjectContactRequest(request,env,ctx){
  ...
  const propertyAttribution=await resolvePropertyOpportunityAttribution(env,b.propertyOpportunitySlug);
  const source=propertyAttribution?"Property Intelligence":"Start a Project — Contact Me";
  ...
  const requested=cleanString(b.reference,80),linkedReference=propertyAttribution&&/^EUS-HOME-\d{8}-[A-F0-9]{7,8}$/.test(propertyAttribution.convertedLeadId)?propertyAttribution.convertedLeadId:"",reference=/^EUS-HOME-\d{8}-[A-F0-9]{8}$/.test(requested)?requested:(linkedReference||projectReference("HOME")),now=new Date().toISOString();
  const propertyIntelligence=propertyAttribution?{opportunityId:propertyAttribution.opportunityId,subtype:propertyAttribution.subtype,campaign:propertyAttribution.campaign,propertySource:propertyAttribution.propertySource,scoreConfidence:propertyAttribution.scoreConfidence,initialScores:{solar:propertyAttribution.solarScore,lithium:propertyAttribution.lithiumScore,combined:propertyAttribution.combinedScore},conceptViewed:propertyAttribution.conceptViewed,qrScanCount:propertyAttribution.qrScanCount}:{};
  const incomingDetails={...cleanedDetails,journeyId,intakeIntent,contactRequestedAt:now,leadSource:source,...(propertyAttribution?{propertyIntelligence}:{})},details=await mergeProjectIntakeDetails(env,reference,incomingDetails),summary=handymanSummary(details);
  await env.LEADS_DB.prepare(`INSERT INTO project_opportunities (reference,project_family,customer_name,phone,email,preferred_contact,consent,city,zip,state,service_area,intake_status,opportunity_status,project_category,summary,source,next_action,details_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(reference) DO UPDATE SET customer_name=excluded.customer_name,email=excluded.email,preferred_contact='Email',consent=1,city=CASE WHEN excluded.city<>'' THEN excluded.city ELSE project_opportunities.city END,zip=CASE WHEN excluded.zip<>'' THEN excluded.zip ELSE project_opportunities.zip END,state=excluded.state,service_area=excluded.service_area,intake_status=CASE WHEN project_opportunities.intake_status='submitted' THEN project_opportunities.intake_status ELSE 'Incomplete / Contact Requested' END,opportunity_status=CASE WHEN project_opportunities.opportunity_status IS NULL OR project_opportunities.opportunity_status='' OR project_opportunities.opportunity_status IN ('potential','submitted','outside_area_review','new') THEN 'new' ELSE project_opportunities.opportunity_status END,project_category=excluded.project_category,summary=excluded.summary,source=excluded.source,next_action=CASE WHEN project_opportunities.next_action IS NULL OR project_opportunities.next_action='' OR project_opportunities.next_action IN ('Review Potential Project','Review Submitted Project','Outside Area Review','Email Customer') THEN 'Email Customer' ELSE project_opportunities.next_action END,details_json=excluded.details_json,updated_at=excluded.updated_at`)
    .bind(reference,"home",name,"",email,"Email",1,city,zip,state,area.serviceArea,"Incomplete / Contact Requested","new","Small Repairs & Handyman",summary,source,"Email Customer",JSON.stringify(details),now,now).run();
  if(propertyAttribution)await linkPropertyOpportunityLead(env,propertyAttribution.opportunityId,reference,false);
  const ownerNotification=await scheduleOwnerLeadNotification(env,ctx,projectOwnerNotificationSpec({reference,projectType:"home",intakeIntent,contact:{name,phone:"",email,preferredContact:"Email"},category:"Small Repairs & Handyman",summary,city,zip,state,serviceArea:area.serviceArea,nextAction:"Email Customer",source}));
  if(env.MARKETPLACE_DB&&sessionId){
    await recordSiteEvent(env,{eventType:"lead_submitted",eventValue:"home",sessionId,reference,page:"/start-a-project",details:{projectType:"home",intakeIntent,serviceArea:area.serviceArea,source,status:"new",journeyReference:journeyId,propertyOpportunity:propertyAttribution?.opportunityId||""}},{serverConfirmed:true,request}).catch(()=>{});
  }
...
async function handleProjectSubmit(request,env,ctx){
  ...
  const intakeIntent=canonicalProjectIntakeIntent(b.intakeIntent,projectType);
  if(!intakeIntent)return jsonResponse({error:"Invalid Project intake intent"},400);
  const city=cleanString(b.city,120),zip=normalizeProjectZip(b.zip),state=normalizeProjectState(b.state),contact=safeContactPayload(b),journeyId=cleanString(b.journeyId,120),sessionId=cleanString(b.sessionId,100);
  const propertyAttribution=await resolvePropertyOpportunityAttribution(env,b.propertyOpportunitySlug);
  const source=propertyAttribution?"Property Intelligence":(cleanString(b.source,120)||"start-a-project");
  const rawDetails=(b.details&&typeof b.details==="object"&&!Array.isArray(b.details))?b.details:{};
  const handymanDetails=intakeIntent==="Small Repair / Handyman"?cleanHandymanDetails(rawDetails):null;
  ...
  const requested=cleanString(b.reference,80),linkedReference=propertyAttribution&&/^EUS-(HOME|RV|SOLAR)-\d{8}-[A-F0-9]{7,8}$/.test(propertyAttribution.convertedLeadId)?propertyAttribution.convertedLeadId:"",reference=/^EUS-(HOME|RV|SOLAR)-\d{8}-[A-F0-9]{8}$/.test(requested)?requested:(linkedReference||projectReference(projectType.toUpperCase())),now=new Date().toISOString(),opportunityStatus="new",nextAction=(area.serviceArea==="outside_standard_area"||area.serviceArea==="manual_review")?"Verify Service Area":"Call Customer",propertyIntelligence=propertyAttribution?{opportunityId:propertyAttribution.opportunityId,subtype:propertyAttribution.subtype,campaign:propertyAttribution.campaign,propertySource:propertyAttribution.propertySource,scoreConfidence:propertyAttribution.scoreConfidence,initialScores:{solar:propertyAttribution.solarScore,lithium:propertyAttribution.lithiumScore,combined:propertyAttribution.combinedScore},conceptViewed:propertyAttribution.conceptViewed,qrScanCount:propertyAttribution.qrScanCount}:{},incomingDetails={...(handymanDetails||rawDetails),journeyId,intakeIntent,...(propertyAttribution?{propertyIntelligence}:{})},details=await mergeProjectIntakeDetails(env,reference,incomingDetails);
  try{
  ...
  if(propertyAttribution)await linkPropertyOpportunityLead(env,propertyAttribution.opportunityId,reference,true);
  const ownerNotification=await scheduleOwnerLeadNotification(env,ctx,projectOwnerNotificationSpec({reference,projectType,intakeIntent,contact,category,summary,city,zip,state,serviceArea:area.serviceArea,nextAction,source}));
  if(env.MARKETPLACE_DB&&sessionId){
    await recordSiteEvent(env,{eventType:"lead_submitted",eventValue:projectType,sessionId,reference,page:"/start-a-project",details:{projectType,intakeIntent,serviceArea:area.serviceArea,source,status:opportunityStatus,journeyReference:journeyId,propertyOpportunity:propertyAttribution?.opportunityId||""}},{serverConfirmed:true,request}).catch(()=>{});
  }
