...
async function adminSnapshot(db, adminEmail) {
  ...
  const batchMetrics=batches.map(b=>{
    const rows=orders.filter(o=>o.batch_id===b.batch_id);let units=0,merchandise=0,supplier=0,weight=0,volume=0,blocked=0,pendingConfirm=0,allocated=0,ready=0;
    ...
    const target=Number(b.target_units||0);const fill=target>0?Math.min(999,Math.round((units/target)*1000)/10):null;const freight=b.freight_accessorial_cost_cents??b.quote_amount_cents??allocated??0;const contribution=merchandise-supplier-Number(freight||0);
...
async function upsertDestination(request,db,adminEmail){
  ...
  const blockers=routeApprovalBlockers(record,{...raw,destination},eligibility);if(blockers.length)return json({error:"Route approval blocked",blockers},409);
  ...
  const costs=costKeys.map(k=>nullableInt(raw[k]));const sum=costs.reduce((s,v)=>s+Number(v||0),0);const landed=nullableInt(raw.totalLandedCostCents)??sum;const retail=nullableInt(raw.retailPriceCents);const customerShip=nullableInt(raw.customerShippingChargeCents);const contribution=nullableInt(raw.estimatedGrossContributionCents)??(retail===null?null:retail+Number(customerShip||0)-landed);
...
async function batchBlockers(db,batchId){
  ...
  for(const o of rows){const prefix=`${o.sku||o.request_id}: `;if(o.hold){blockers.push(prefix+"line is on HOLD");continue;}const sr=await db.prepare("SELECT * FROM eus_lithium_shipping_records WHERE sku=? COLLATE NOCASE LIMIT 1").bind(o.sku).first();if(!sr){blockers.push(prefix+"exact shipping SKU record is missing");continue;}if(!sr.catalog_product_id||!sr.supplier_sku)blockers.push(prefix+"exact Catalog/supplier identity is incomplete");if(sr.hold)blockers.push(prefix+"product is on HOLD");if(!["AVAILABLE","LOW"].includes(sr.supplier_stock_state)||!sr.inventory_last_confirmed||sr.inventory_recheck_required)blockers.push(prefix+"supplier inventory requires current confirmation");if(sr.damage_recall_notes)blockers.push(prefix+"damage/recall note requires review");
