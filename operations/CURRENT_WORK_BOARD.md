...
**Last source-state reconciliation baseline:** `c2aee08412834ba4203a915eaa40d91616f2c80b`  
...
- **store / catalog / product-item / shopability / commerce-navigation work may move**;
...
The current synced developer route is `HOMEPAGE_STORE_MERCHANDISING_WORK_ORDER_2026-09-12.md`, read through the newest owner directive above.
...
| VEVOR Supplier / Catalog / Shopify / Fulfillment | VEVOR Project Operations Manager | VEVOR Reconciliation & Price-Control Specialist | [VEVOR Project Source](./vendor-project-sources/VEVOR_PROJECT_SOURCE.md) | ACTIVE / 19 A-tier live / **17 of 17 qualified B-tier ACTIVE** / 10-SKU fresh check COMPLETE / **3 orderable + 7 out of stock** / exact PRO economics pending for the 3 viable SKUs |
...
|---|---|---|---|---|---|---|---|
| Direct-site profitable first sale / Shopify + PayPal | **MPM / Company Operations**; Shopify + vendor Projects + MASTER DEVELOPER only where technical | ACTIVE — NATIVE SHOPIFY CHECKOUT LIVE / CUSTOM PAYPAL LIVE / OWNED-TRAFFIC TESTING ACTIVE / FIRST REAL PAID ORDER OPEN | **P0 PRIMARY PROFIT LANE** | Checkout is not the current blocker. Qualified traffic to economically cleared offers is the bottleneck. SOK direct-site testing remains protected; non-SOK cash-light Apparel testing has also begun without changing SOK. | Preserve both working checkout paths. Route owned/free traffic only to exact verified positive-contribution offers. On a real order, reverify SKU/source/price/sellability, confirm payment + durable order record, route supplier fulfillment and record realized contribution. | Repeatable paid-order → source → fulfillment → contribution loop is proven and first real direct-site paid order is routed cleanly. | `PROFITABILITY_RECOVERY_WORKFLOW_V1_0.md`; `COMMERCIAL_REVENUE_ACCELERATION_RUN_RECEIPT_2026-09-12.md`; `PM4_NON_SOK_APPAREL_TRAFFIC_RUN_2026-09-12.md` |
| eBay customer/cash recovery + Phase 1 profitability contraction | **eBay Store Operations Worker / Specialist** under Peter Torres / Company Operations | ACTIVE — AUTHENTICATED SELLER HUB LIVE READS / 4 AWAITING SHIPMENT / WEED-WACKER CANCELLATION PROCESSING / MUTATION CLICKS GATED ONLY | **P0 PARALLEL** | Opera recovered on allowed retry and authenticated live reads are available. Current Awaiting Shipment remains organizer + spotlight + two separate folding-bed/cot orders. Weed-wacker cancellation is still processing. The remaining tooling gate is consequential Seller Hub mutation clicks, not authentication/read access. | Customer obligations first → cash release → action-capable Seller Hub for approved stop-loss/listing mutations → current-cost reverify solar charger → verify gazebo + mattress source → zero-demand purge → verify small profitable core. Never resubmit the weed-wacker cancellation or double-refund. | Customer obligations terminally resolved or cleanly shipped/tracked; held-cash blockers cleared/isolated; failed configurations cannot create repeat losses; active revenue core is exact-source/executable/positive-contribution. | `EBAY_STORE_OPERATIONS_CURRENT_WORKTREE.md`; `EBAY_SPECIALIST_POST_APPROVAL_RUN_RECEIPT_2026-09-12.md`; profitability management decision |
| VEVOR direct-site first profitable order | **VEVOR Project Operations Manager**; Peter oversight; VEVOR Specialist + bounded Shopify workers | IN_PROGRESS — 19 A-TIER LIVE / **17 OF 17 B-TIER ACTIVE** / 10-SKU FRESH CHECK COMPLETE / **3 ORDERABLE + 7 OUT OF STOCK** | P1 PROFITABLE FIRST ORDER | Current-window public sellability/price-MAP work is complete. The three orderable SKUs are `XXKLJT124INCLJF0QV0`, `AXLSTCQJDSYKAZ99C001V0`, and `D25FT14IN20AHOGLOV1`; all remain `HOLD — ECONOMICS UNKNOWN` pending exact current PRO net unit price + normal continental-U.S. dropship shipping treatment. Seven shortlist SKUs are out-of-stock holds. | **Do not rerun the 10-SKU fresh check this source window.** Reconcile protected economics for the three viable SKUs only → calculate contribution → disposition `PROMOTE` or exact HOLD → route clean winners to owned/free traffic → first real order proof. | First real VEVOR order completes payment → live recheck → VEVOR purchase → tracking → customer delivery with positive/reconciled contribution and repeatable handling. | [VEVOR Project Source](./vendor-project-sources/VEVOR_PROJECT_SOURCE.md); `VEVOR_FIRST_SALE_FRESH_CHECK_2026-09-12.md`; `DIRECT_SITE_PROFITABILITY_GATE_2026-09-11.md` |
| Elevation Apparel cash-light revenue / provider economics | **Apparel Vendor Operations Manager** under Peter Torres; Shopify/Fourthwall execution lanes as applicable | ACTIVE — FOURTHWALL 29 PRODUCTS / NATIVE FULFILLMENT PROVEN / MOUNTAIN PATCH DIRECT OWNED-TRAFFIC TEST / SPREADCONNECT DISCONNECTED FROM SHOPIFY | P1 PARALLEL CASH-LIGHT REVENUE | Fourthwall is a proven no-inventory-prebuy fulfillment lane; payout readiness remains open. Mountain Patch Baseball Cap direct Fourthwall test is active/scheduled. Shopify live-state proved Spreadconnect is not currently installed and has no Spreadconnect fulfillment-service location. Direct Fourthwall economics do not equal TikTok Affiliate economics. | Verify Fourthwall post publication/traffic/order evidence; audit 29 products; normalize/reprice weak hero economics; set payout readiness; install/auth Spreadconnect only through the proper Shopify + NA account path when authorized; then compare exact branded blanks/cost/shipping and return 3–5 launch-ready Shopify apparel SKUs with one fulfillment owner each. | 3–5 creator-ready Apparel heroes have verified provider economics, safe purchase/fulfillment path and positive expected contribution; first new Apparel order records realized contribution. | `APPAREL_VENDOR_OPERATIONS_CURRENT_WORKTREE.md`; `FOURTHWALL_STARTUP_CAPITAL_REVENUE_PROGRAM_2026-09-12.md`; `SHOPIFY_SPREADCONNECT_ELEVATION_GEAR_EXECUTION_RETURN_2026-09-12.md`; `PM4_NON_SOK_APPAREL_TRAFFIC_RUN_2026-09-12.md` |
| TikTok Affiliate Growth / profitable creator engine | **TikTok Affiliate Growth Manager** under Peter Torres; Apparel/vendor lanes supply exact economics | ACTIVE — LIVE AFFILIATE ACCESS / PRODUCTS AUTO-ENROLLED IN OPEN COLLABORATION BY DEFAULT / PROFITABILITY GATE ACTIVE / SELLER-VERIFICATION APPEAL SEPARATE AND TERMINAL-WAITING | P1 PARALLEL ACQUISITION | Auto-enrollment creates possible commission exposure even without a deliberate new campaign. `AUTO-ENROLLED` is not `PROFIT-APPROVED`. Mountain Patch Baseball Cap remains a direct Fourthwall `PROMOTE TEST` but is `HOLD RATE / REPRICE OR REROUTE` for TikTok Affiliate under the current screen. | Audit live Open Collaboration commission settings through owning TikTok Affiliate lane; do not intentionally broaden margin-unknown creator traffic; validate exact fee/provider/shipping/promo/sample/return economics and safe commission ceiling per SKU; Trucker Cap next; tee/Crop Tee/Essential Hoodie remain hold/reprice/reroute. | At least one exact hero SKU supports a safe creator offer and produces attributable positive realized contribution; creator ladder/performance loop becomes repeatable. | `TIKTOK_AFFILIATE_GROWTH_CURRENT_WORKTREE.md`; `TIKTOK_AFFILIATE_PROFITABILITY_EXECUTION_2026-09-12.md`; `PM4_TIKTOK_AFFILIATE_LIVE_RECON_2026-09-12.md` |
...
| Homepage-connected commerce / protected-top boundary | **MASTER DEVELOPER** under PM4 / Company Operations | ACTIVE BOUNDARY — COMMERCE CAN MOVE / PROTECTED TOP HOMEPAGE HARD LOCKED | P2 BOUNDED REVENUE TECHNICAL | Store/catalog/shopability/navigation work is authorized, including homepage-connected commerce only when protected-top rendered/runtime output remains unchanged. Shared API/catalog/backend changes count as protected-top changes if they alter that output. | Use current owner directive + synced homepage work order. Before shared-dependency commerce change: `CHECK PROTECTED TOP DEPENDENCY → VERIFY TOP OUTPUT UNCHANGED → EXECUTE`. If protected-top output would change, hold only that mutation and route exact delta to Casey while continuing other commerce work. | Commerce improvements ship without changing protected-top layout, imagery, copy, CTAs, product bindings, styling, timing, responsive behavior or runtime output. | `OWNER_DIRECTIVE_HOMEPAGE_LOCK_2026-09-12.md`; `HOMEPAGE_STORE_MERCHANDISING_WORK_ORDER_2026-09-12.md`; `OS_RECON_OWNER_DIRECTIVE_SYNC_2026-09-12.md` |
...
| ElevationUpScales.com routing/content integrity residual | **MASTER DEVELOPER** under authorized management | RECON CONFIRMED — CORE COMMERCE HEALTHY / ROUTING + LEGACY COPY DRIFT DEGRADED | P2 TECHNICAL RESIDUAL | Live RECON confirmed core store, Start a Project, Solar Builder and checkout are usable. `/report-an-issue` routes to Store instead of an issue surface. `/marketplace` and `/make-a-listing` resolve into Store while stale public language still advertises the old community Marketplace/Create Listing concept. `/universal/local-products.js` returned 404; defect status depends on whether current active source still references it. Final Hawaii pre-payment freight gate still requires exact verification. Protected-top homepage boundary blocks only exact changes that would alter protected-top output. | Bounded technical/source reconciliation only: fix or formally retire Report-an-Issue path; reconcile legacy route/copy on authorized surfaces; verify/remove any active `local-products.js` reference; verify Hawaii freight gate before payment; run canonical smoke. Check shared dependencies against protected-top output before execution. | Public routes on authorized surfaces match current operating model, no active broken legacy asset reference exists, Hawaii freight cannot silently reach an unsafe payment path, and protected-top output remains unchanged. | `OS_RECON_ELEVATIONUPSCALES_DOTCOM_2026-09-11.md`; issue #65 / current application; current owner homepage directive |
...
Do **not** treat earlier eBay startup text saying authenticated Seller Hub was unavailable as current. Authenticated Seller Hub live reads are current; only unsupported consequential mutation clicks remain gated.
...
15. Commerce work may continue while the protected top homepage remains unchanged; hold only exact conflicting top-output mutations.
