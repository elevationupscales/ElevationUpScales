...
| Checkout-gate residual — exact Renogy delayed-order test only | Commerce / Developer | IN_PROGRESS — SHARED + DOBA GENERIC GATES LIVE-CLOSED | P2 RESIDUAL | Shared Buy Now source routing and generic Doba availability presentation are production-closed. Remaining test is exact Renogy delayed-order SKU under current active/backorder controls. | Do not rewrite shared checkout/Doba logic. Test only a verified exact Renogy delayed-order SKU through product/cart/checkout/production proof without bypassing exact dealer orderability controls. | Exact delayed-order path passes without a vendor-wide bypass. | `UNIVERSAL_CHECKOUT_SOURCE_ROUTING_LIVE_RECEIPT_2026-09-11.md`; `DOBA_AVAILABILITY_PRESENTATION_LIVE_RECEIPT_2026-09-11.md`; `RENOGY_CURRENT_WORKTREE.md` |
