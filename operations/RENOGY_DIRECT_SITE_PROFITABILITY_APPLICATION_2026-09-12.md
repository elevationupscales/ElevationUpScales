...
**VERIFIED SKU → CURRENT PUBLIC PRICE / RENOGY PRICE CONTROL → CURRENT DEALER COST → CURRENT ORDERABILITY → APPROVED MEDIA → FULL VARIABLE COST CHECK → POSITIVE CONTRIBUTION → ACTIVATE / PROMOTE → CUSTOMER ORDER → REVERIFY → FULFILL → RECORD ACTUAL PROFITABILITY → SCALE.**

A Renogy SKU is not promotion-ready merely because it is MAP-compliant, orderable, staged in Shopify, or capable of checkout.
...
## Current Renogy supplier economics source

Renogy Sales Support supplied the current item workbook with protected dealer price and MSRP/public reference fields. Dealer pricing remains protected and must not be copied into public storefront copy or exposed in public customer surfaces.

Renogy supplier guidance also states Lower-48 shipping is free under the current dealer program. Reverify this at order time and do not infer Alaska, Hawaii, territory, international, or dangerous-goods freight from the Lower-48 rule.

## Current launch-wave profitability reconciliation

### `RSP100DCT-US` → supplier item `RSP100DCT-G1-US`

- Current Shopify/customer price previously verified: **$99.99**.
- Current supplier workbook dealer cost: **$84.99**.
- Pre-processor / pre-platform contribution spread: **$15.00**.
...
- Exact approved Renogy media path: established.
- Payment processor / Shopify per-order treatment: not yet documented in this Renogy lane strongly enough to close the company profitability gate.

**Current state: `HOLD — ECONOMICS UNKNOWN`** until exact orderability plus applicable checkout/platform fee treatment is verified. The SKU has a positive pre-fee spread but is not yet authorized for promotion solely from that spread.

### `RBM500-US` → supplier item `RBM500-G3-US`

- Current Shopify/customer price previously verified: **$87.99**.
- Current supplier workbook dealer cost: **$59.99**.
- Pre-processor / pre-platform contribution spread: **$28.00**.
- Lower-48 supplier shipping guidance: free, subject to exact order-time verification.
- Current dealer orderability/backorder authority: still requires authenticated Partner Portal exact-SKU confirmation.
...
Under the new company workflow, launch ranking must consider:
...
This means Renogy's previous merchandising order is no longer controlled only by product appeal or launch cleanliness.

Based on the currently verified supplier workbook and customer prices, `RBM500-US` has the stronger pre-fee dollar-contribution buffer and remains a low-friction monitoring/cross-sell product. It should receive high profitability priority once exact media and dealer orderability clear.

`RSP100DCT-US` remains strategically useful as the first solar-panel trust product, but its narrower pre-fee spread requires more care with processor/platform costs and any future discounts.

Do not create a discount on either SKU until the expected contribution after all applicable variable costs remains positive and Renogy price controls are preserved.

## Order-time recheck

For every Renogy vendor-fulfilled order:

**PAID ORDER → REVERIFY EXACT SKU → REVERIFY CURRENT DEALER COST → REVERIFY SELLABILITY / BACKORDER AUTHORITY → REVERIFY LOWER-48 SHIPPING TREATMENT → CALCULATE CURRENT EXPECTED CONTRIBUTION → PLACE RENOGY ORDER → CAPTURE ACCEPTANCE / TRACKING → RECORD ACTUAL ORDER ECONOMICS.**
...
The 188-product Renogy prospect universe is now subject to profitability screening as part of catalog prioritization.

For every candidate, maintain protected internal fields for:
...
Do not place protected dealer cost or private economics into public Git surfaces intended for external consumption. This operating receipt records only the minimum economics necessary for internal control and must remain treated as vendor-operational data.
...
4. Calculate final expected order contribution for both launch SKUs.
...
7. On first real Renogy order, record actual contribution and use actuals to refine future Renogy ranking.

**Control phrase:**

**RENOGY EXACT SKU → PRICE CONTROL → DEALER COST → ORDERABILITY → MEDIA → FULL VARIABLE COSTS → POSITIVE CONTRIBUTION → ACTIVATE / PROMOTE → RECORD ACTUALS → SCALE.**